...
foreach ($virtualPath in $virtualPaths) {
    if ($appName -ne '') {
        CreateVirtualDirectory -WebsiteName $websiteName -VirtualPath $virtualPath -PhysicalPath $physicalPath -UserName $userName -Password $password -AppName $appName
    }
    else {
        CreateVirtualDirectory -WebsiteName $websiteName -VirtualPath $virtualPath -PhysicalPath $physicalPath -UserName $userName -Password $password
    }
}
